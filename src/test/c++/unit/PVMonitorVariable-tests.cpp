...
TEST(PVMonitorVariable, GetValue_error)
{
  ...
  if (status)
    { // Setup implicit with AddAttribute
...
TEST(PVMonitorVariable, GetValue_success)
{
  ...
  if (status)
    { // Update PVA variable
      ...
      status = (ccs::HelperTools::SetAttributeValue(_pva_srv->GetVariable("sequencer::test::pva-variable"), "scalars.uint32", value) &&
                ccs::HelperTools::SetAttributeValue(_pva_srv->GetVariable("sequencer::test::pva-variable"), "arrays.uint32[1]", value));
    }
  ...
  if (status)
    { // Test variable
      status = (1234u == ccs::HelperTools::GetAttributeValue<ccs::types::uint32>(_pva_srv->GetVariable("sequencer::test::pva-variable"), "scalars.uint32"));
    }
